...
Build with optimization or the numbers are meaningless; each file carries its
own command line. Timing runs want `-march=native`, which is *not* what the
test suite is built with — see [`docs/reproducibility.md`](../docs/reproducibility.md)
for why that distinction matters when comparing results.
