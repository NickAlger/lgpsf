...
| [`fitting_defaults.py`](fitting_defaults.py) | **What is the initial-guess ladder buying?** Sweeps `num_rungs`, the circle-rung gate and the window-shape family over five prior qualities. PROVISIONAL: nothing in the ladder changed an answer, and the reason turns out to be that a bad prior binds through the WINDOW rather than the initialization. Results in [`fitting-defaults.md`](fitting-defaults.md). |
| [`anisotropy_hardening.py`](anisotropy_hardening.py) | **Does the initial-guess ladder have an anisotropy blind spot?** The frog hardened to 8:1 — the only problem here anisotropic enough to ask. Nothing in the dictionary covers orientation independently of the prior, and the Python prototype found that this bites. It reproduces under **free** mu and not under the pinned default, and a circular start does not bias the fitted answer toward roundness. Kept as the guard that would catch a future default re-arming mu release. Results in [`anisotropy-hardening.md`](anisotropy-hardening.md). |
| [`lm_tolerance.py`](lm_tolerance.py) | **How tight does the nonlinear solver need to be?** The fit runs to `1e-8`; the selection score lives at `1e-1`. Sweeps `ftol`/`xtol`/`gtol` and the evaluation cap over five decades, at two rung counts. `ftol` is the only test that binds, accuracy does not move at all, and the apparent gains at the loose end turn out to be the mode ladder flipping depth. Results in [`lm-tolerance.md`](lm-tolerance.md). |
