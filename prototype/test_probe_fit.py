"""Tests for probe_fit.py (the candidate-stream fitting layer),
probe_moments.py, and init_dictionary.window_shape.
...
raw-data contract fit_from_probes and backproject/raw_moments document,
...
Run directly (`python test_probe_fit.py`) or via pytest.
"""
import os
import sys
...
from init_dictionary import window_shape
from probe_fit import ProbeFitConfig, ProbeFitResult, fit_from_probes
from probe_moments import backproject, raw_moments
...
def _make_row(rng, K=90, k=60, mu_true=None, s_true=0.7):
    ...
    spike_index = int(np.argmin(np.linalg.norm(x, axis=0)))
    if mu_true is None:
        mu_true = x[:, spike_index].copy()
    ...
    row_mass = m2[spike_index]
    raw_row = row_mass * m2 * phi
    raw_row[spike_index] += row_mass * s_true
    z = rng.standard_normal((k, K))
    y = z @ raw_row
    return dict(x=x, m2=m2, z=z, y=y, spike_index=spike_index,
...
def test_fit_from_probes_recovers_synthetic_fixed_mu():
    ...
    prob = _make_row(rng)
    result = fit_from_probes(prob["x"], prob["m2"], prob["z"], prob["y"],
                     prob["mu_true"], MODES,
...
def test_fit_from_probes_release_recovers_offset_center():
    ...
    res_rel = fit_from_probes(prob["x"], prob["m2"], prob["z"], prob["y"], mu0,
                      MODES, spike_index=prob["spike_index"],
                      config=ProbeFitConfig(mu="fixed_then_release",
                                          target_score=None))
    ...
    res_fix = fit_from_probes(prob["x"], prob["m2"], prob["z"], prob["y"], mu0,
                      MODES, spike_index=prob["spike_index"],
                      config=ProbeFitConfig(mu="fixed", target_score=None))
    assert res_rel.score < res_fix.score   # releasing genuinely helped


def test_fit_from_probes_backprojection_workflow_rescues_bad_center():
    ...
    res_bad = fit_from_probes(prob["x"], prob["m2"], prob["z"], prob["y"], mu0_bad,
                      MODES, spike_index=prob["spike_index"])
    assert res_bad.score > 1e-3   # stuck short of exact

    r_hat = backproject(prob["z"], prob["y"])
    mu_bp, sigma_bp = raw_moments(prob["x"], r_hat,
                                  spike_index=prob["spike_index"],
                                  rel_threshold=0.05, noise_mad=3.0)
    res_wf = fit_from_probes(prob["x"], prob["m2"], prob["z"], prob["y"], mu_bp,
                     MODES, spike_index=prob["spike_index"], sigma0=sigma_bp)
...
def test_fit_from_probes_sigma0_tried_first():
...
def test_fit_from_probes_mixed_ladder_enumeration():
    ...
    res = fit_from_probes(prob["x"], prob["m2"], prob["z"], prob["y"],
                  prob["mu_true"], MODES, spike_index=prob["spike_index"],
                  config=ProbeFitConfig(mu="fixed", target_score=None))
    ...
    res2 = fit_from_probes(prob["x"], prob["m2"], prob["z"], prob["y"],
                   prob["mu_true"], MODES, spike_index=prob["spike_index"],
                   config=ProbeFitConfig(mu="fixed", target_score=None,
...
def test_fit_from_probes_mode_ladder_selects_simplest():
...
def test_fit_from_probes_counting_rule_skips_levels():
...
def testwindow_shape_mass_weighted_geometry():
    """window_shape must measure the window REGION's shape, independent
    ...
    for x, m in [(x1, m1), (x2, m2)]:
        asp, vmax = aspect_and_dir(window_shape(x, m))
...
def test_raw_moments_mass_quadrature():
    """The mass subtlety: raw row values already carry the lumped-mass
    quadrature weight, so raw_moments needs no mass vector -- moments
    ...
    def moments_on(x, m):
        ...
        return raw_moments(x, raw, rel_threshold=0.0)
...
def test_raw_moments_diag_exclusion():
    """A diagonal spike left inside the weights drags mu toward the node
    and shrinks Sigma; spike_index must neutralize it."""
    rng = np.random.default_rng(4)
    ...
    spike_index = int(np.argmin(np.linalg.norm(x - 0.9, axis=0)))
    raw_spiked = raw.copy()
    raw_spiked[spike_index] += 50.0 * np.abs(raw).sum()

    mu_bad, S_bad = raw_moments(x, raw_spiked, rel_threshold=0.0)
    mu_ok, S_ok = raw_moments(x, raw_spiked, spike_index=spike_index,
                              rel_threshold=0.0)
...
def test_backprojection_end_to_end():
    """backproject + raw_moments from k random probes approximates the
    ...
    row_mass = prob["m2"][prob["spike_index"]]
    raw_row = row_mass * prob["m2"] * prob["phi"]
    mu_ref, S_ref = raw_moments(prob["x"], raw_row, rel_threshold=0.0)

    mu_hat, S_hat = raw_moments(prob["x"], r_hat,
                                spike_index=prob["spike_index"],
                                rel_threshold=0.05, noise_mad=3.0)
...
if __name__ == "__main__":
    ...
    test_fit_from_probes_backprojection_workflow_rescues_bad_center()
    test_fit_from_probes_sigma0_tried_first()
    ...
    print("all probe_fit checks passed")
