"""Generate include/lgpsf/detail/lg_harmonics_table.hpp.

OFFLINE CODE GENERATOR. Run it only to change the table's extent (DIMENSIONS,
MAX_ELL below); the generated header is committed, and an ordinary build never
invokes this script. Python is used here because the construction needs exact
rational arithmetic, which `fractions.Fraction` supplies and C++ does not.

    python tools/generate_lg_harmonics_table.py
...
r^ell * {cos, sin}(ell*theta), the angular part `eval_lg` writes out directly).
This script builds that basis for ell = 0..MAX_ELL and N in DIMENSIONS and
emits it as a literal table, so the runtime evaluation code in
harmonic_polynomials.hpp needs no linear-algebra dependency and no
...
if __name__ == "__main__":
    ...
    hpp_path = os.path.normpath(
        os.path.join(here, os.pardir, "include", "lgpsf", "detail",
                     "lg_harmonics_table.hpp"))
    write_table_header(table, hpp_path)
