...
```text
8:1 frog with a signed modulation (a_mod = 2), 60 probes, center released

row                   3 circles (default)            your sigma only   ratio
(0.55, 0.35)       0.1088 (23 fits, 15 modes)     0.0516 (11 fits, 21 modes)   2.11x
(0.4, 0.6)         0.0716 (23 fits, 15 modes)     0.0359 (11 fits, 21 modes)   1.99x
(0.62, 0.48)       0.1118 (23 fits, 15 modes)     0.0637 (11 fits, 21 modes)   1.75x
(0.35, 0.45)       0.0514 (23 fits, 21 modes)     0.0514 (11 fits, 21 modes)   1.00x

A single well-chosen guess is never worse, wins on 3 of 4 rows here (up to 2.1x),
and costs less than half the fits. It is not magic: on the rows where it ties,
the circles had already found the same optimum.

oriented_ladder: 12 guesses over 4 angles x 3 scales -> 0.0481 (77 fits, 21 modes)
Use it when the shape is anisotropic and you cannot say which way it points.
...
*Generated by `docs/generate_examples.py` from [`examples/initial_guesses.py`](../../examples/initial_guesses.py); the output and figures above come from actually running it.*
