...
```text
PCG on the TRUE system (Hd + a Hr), a = 3e-05; iterations to 1e-6 / 1e-10.
Each preconditioner is the corrected fit, applied exactly (Cholesky backend).

  prior only (a Hr)                          PCG  89 / 142
  certified fit, no deflation                PCG  14 /  21   
  + deflate_free (rcond 3e-2)                PCG  23 /  35   [kept 20, clamped 2]
  + value_pass V1, m=30 true heat solves     PCG  13 /  21   [kept 20, clamped 0, theta in [-0.13, 0.78]]
  + value_pass V2 (power step), m=30         PCG  10 /  17   [applies 30, kept 30]

The free pass is worse than no deflation at all: its values come from
the same 20 residuals that defined the directions, and pricing 20
directions from 20 in-sample equations is interpolation, not estimation.
The reports said so before the counts did -- clamps, and estimates far
outside the true range the value pass measures. V1 buys exact values but
saturates at the 20-dimensional free basis; V2 reaches past it and is
the only construction that beats the uncorrected fit here. This is the
field-scale allocation lesson in miniature: residual DIRECTIONS are
nearly free, trustworthy VALUES cost true applies -- spend there. The
pairs V2 bought are now in `archive.Q_vp`, kept because a rebuild at a
new shift can reuse them without touching the operator again.
```
...
*Generated by `docs/generate_examples.py` from [`examples/deflation.py`](../../examples/deflation.py); the output and figures above come from actually running it.*
