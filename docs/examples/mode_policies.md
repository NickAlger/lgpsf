...
No policy wins everywhere, and note that the operator-layer DEFAULT --
WedgeLadder(10, 2) -- is not the winner on this target at any budget.
That is the lesson, not a bug: the frog kernel's modulation is strongly
ANGULAR, so capping |ell| <= 2 throws away exactly the modes it needs.
Relaxing the cap to 4 is competitive, and complete shells win outright.

The default was chosen against a glaciology Hessian whose rows are much
more nearly elliptical, where the wedge reaches the same accuracy for a
sixth of the modes (docs/validation.md). If your target has angular
structure, say so with the policy -- that is what the axis is for.
```
...
*Generated by `docs/generate_examples.py` from [`examples/mode_policies.py`](../../examples/mode_policies.py); the output and figures above come from actually running it.*
