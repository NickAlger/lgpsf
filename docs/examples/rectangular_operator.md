...
```text
truth is 225 x 900 -- 15x15 rows, 30x30 columns
fitted operator is 225 x 900, 225 rows carrying a model
...
*Generated by `docs/generate_examples.py` from [`examples/rectangular_operator.py`](../../examples/rectangular_operator.py); the output and figures above come from actually running it.*
