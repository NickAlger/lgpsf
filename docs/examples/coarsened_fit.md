...
![coarsened_fit.png](../img/coarsened_fit__coarsened_fit.png)

## Output

```text
window: 31005 points at spacing 0.0045; truth 1-sigma axes 0.10 x 0.04 (22 x 9 spacings),
prior 0.10 x 0.08; 60 probes. Scores: 'own' on the quadrature the fit ran on,
'on full' the same model re-scored on the full window.

fit on       cells   wall  speed  1-sigma axes    angle    c[0]   c[1]   c[2] s/true     own on full modes
truth                             0.1000 x 0.0400  30.0   1.000  0.250 -0.300  1.000
full window  31005  4.25s   1.0x  0.1029 x 0.0405  31.1   0.990  0.251 -0.289  1.043  0.0479  0.0479     6
eps 0.05     14642  1.76s   2.4x  0.1028 x 0.0406  31.0   0.989  0.250 -0.290  1.042  0.0488  0.0481     6
eps 0.1       5885  0.77s   5.5x  0.1039 x 0.0405  31.2   0.984  0.251 -0.289  1.028  0.0529  0.0480     6
eps 0.2       2142  0.32s  13.4x  0.1047 x 0.0398  31.8   0.983  0.238 -0.285  1.247  0.0732  0.0491     6

Re-scored on the full window, the coarse fits are within 2.3% of the full one's
score, with the same ellipsoid to about 1% and the same modes, for 6x less wall at
eps 0.1 (the coarsening itself takes milliseconds). Their OWN scores are another
matter: the cells' quadrature error inflates them, by 10% at eps 0.1 and 49% at eps 0.2,
for models that are nearly as good. A score measured on the cells is a search's
internal currency, not a verdict -- which is why fit_operator re-scores its finalists
on the full window before the baseline guard reads them.

wrote examples/coarsened_fit.png
```
...
*Generated by `docs/generate_examples.py` from [`examples/coarsened_fit.py`](../../examples/coarsened_fit.py); the output and figures above come from actually running it.*
