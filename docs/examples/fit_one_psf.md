...
The held-out score is what the library selects on -- it is measured
without the truth, which is all you have on a real problem. The last
column compares against the truth we happen to know, as a check on it.
```
...
*Generated by `docs/generate_examples.py` from [`examples/fit_one_psf.py`](../../examples/fit_one_psf.py); the output and figures above come from actually running it.*
