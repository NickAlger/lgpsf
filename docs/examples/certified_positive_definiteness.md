...
```text
The same fit, counted two ways:
  Euclidean eigenvalues below zero : 164
  pencil eigenvalues below zero    : 164   (deepest -2.1e-05; informed modes reach 0.046)
  pencil below -a0/2 (a0 = 0.001)    : 0
The Euclidean count is discretization noise; the pencil view shows the
tail hugging zero, orders below both the informed modes and the threshold.

make_pd at a0 = 0.001: flipped 0, certified lambda_floor = -2.14e-05
(576 Hr-solves; matvecs and Hr-solves are ALL it used --
no entries, no dense eigensolver)
  dense check, a just above the floor (4.3e-05): min eig +2.1e-05 -> positive definite
  dense check, a just below the floor (1.1e-05): min eig -1.3e-05 -> INDEFINITE
The certificate is exact: definiteness changes exactly where lambda_floor
says it does, and every deployment shift above the floor is safe.

Shrinking a0 toward zero (fresh build each row):
       a0   threshold  flipped  Hr-solves  certified
    1e-03    -5.0e-04        0        576       True
    3e-05    -1.5e-05        1       1152       True
    1e-05    -5.0e-06       10       1152       True
    3e-06    -1.5e-06       22       1152       True

The deeper the threshold reaches into the noise tail, the more modes it
touches and the more the certification costs -- with nothing to show for
it, because the deployment shift absorbs that tail anyway. Exact PSD of
the fit alone (a0 = 0) would mean enumerating the accumulation point at
zero: mode counts and cost growing without bound under mesh refinement.
Anchor a0 at the smallest shift you will deploy, and the flip set stays
...
*Generated by `docs/generate_examples.py` from [`examples/certified_positive_definiteness.py`](../../examples/certified_positive_definiteness.py); the output and figures above come from actually running it.*
