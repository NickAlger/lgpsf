...
is anchored at the target. `Symmetrize.Average` is for operators you KNOW
are symmetric -- a Gauss-Newton Hessian, say -- where it cancels the noise
in the two independent row fits. It is an assembly-time choice that does
not touch the fit, so it cannot warn you when it is the wrong one.
```
