...
```text
576 rows x 576 columns, 576 carrying a model

matvec vs assemble_sparse : max |difference| 0.00e+00
...
Symmetrize.Weighted error 0.4591

Symmetrizing made it 15x WORSE, and correctly so: this operator is
not symmetric (max|H - H^T| / max|H| = 1.00), because the frog kernel's shape
is anchored at the target. Both symmetrizers are for operators you KNOW
are symmetric -- a Gauss-Newton Hessian, say -- where reconciling the two
independent row fits cancels their noise. `Weighted` is the recommendation
there: where the rows disagree about a shared entry, it sides with the row
whose scale the entry actually matters to, instead of letting a strong row's
tail overwrite a weak row's signal. When row scales are comparable, as on
this kernel, the two differ little. Either is an assembly-time choice that
does not touch the fit, so neither can warn you when symmetry itself is
wrong.
```
...
*Generated by `docs/generate_examples.py` from [`examples/deploying_a_fit.py`](../../examples/deploying_a_fit.py); the output and figures above come from actually running it.*
