...
\author{N.~Alger, Claude \quad (notes; session 2026-07-24)}
\date{}
...
machinery (\texttt{lg\_functions.py}, \texttt{ellipsoid\_transform.py}) with
no new derivative math required.
\end{abstract}

\section{Setup}

Recall the discretization picture (session 2026-07-23): $X$ is the discrete
...
in \texttt{lg\_harmonics\_table.py} and \texttt{lg\_functions.py}), the
...
is a direct, discrete correction to specific matrix entries. (Session
2026-07-23 verified this asymmetry reproduces the established $m_\rho
s_\rho\,\delta_{\rho j}$ spike convention exactly.)
...
Euclidean inner product throughout (as an early draft of this design did)
happens to reproduce the correct answer for a single one-hot $e_d$ -- every
...
$X'$ where $\phi_i$ lives in $X$). An earlier pass at this derivation
omitted the $\sqrt{m_\rho}$ on $\what e_d$; this was caught by checking
\eqref{eq:whitened-model} term by term against \eqref{eq:probe-eq} before
implementing it, and confirmed numerically (\texttt{test\_whitening.py}) by
constructing an explicit $H$ from \eqref{eq:row-model} and checking that the
whitened quantities reproduce $y_\ell(\rho)$ to machine precision.
...
The session-2026-07-23 analysis found that projecting a smooth feature
against an extra basis correctly requires the $\Mtwo^{-1}$-weighted inner
...
\end{itemize}
This is the same ``compose with a fixed linear map'' pattern used already
for Stage~1/Stage~2 composition in \texttt{ellipsoid\_transform.py} and for
the extra-basis projection in the 2026-07-23 design discussion. No new
derivative formulas are needed anywhere; \texttt{lg\_functions.py} and
\texttt{ellipsoid\_transform.py} are unchanged.

\section{Resulting architecture}

\begin{itemize}
\item \textbf{Mass-free composed feature layer} (\texttt{lg\_ellipsoid\_feature.py}):
  ...
  about mass matrices.
\item \textbf{Whitening interface layer} (\texttt{whitening.py}): the only
...
\item \textbf{Generic VarPro core} (not yet built): takes whitened probes,
  whitened data, and the whitened-basis eval/JVP/VJP as callables. Performs
  the actual per-row fit (orthogonalize the smooth basis against the extra
  basis, solve the inner linear least squares, form the reduced residual,
  provide its $\theta$-Jacobian to the outer Levenberg--Marquardt loop) in
  ordinary Euclidean terms throughout. Never imports or references a mass
  matrix.
\end{itemize}
