...
- [operator-api-plan.md](operator-api-plan.md) -- AGREED DESIGN, not yet implemented: the whole-operator fitting API (H~ = M1 Phi~ M2 + M1 S two-component type structure, spike convention, inputs incl. the sigma/tau_window resolution, OperatorFit padded arrays, helper table, baseline-always-on guard, symmetry-as-assembly-policy).
