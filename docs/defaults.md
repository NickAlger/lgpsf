...
| `coarsen_above` | `3000` | Coarsen the *fit's* quadrature on windows with more points than this; `0` never does. A per-row work bound that needs no estimate of the kernel's width: graded cells at most `coarsen_eps` times their distance from the centre, each reduced to its mass-weighted centroid — about `3π/eps²` cells per dyadic annulus, so logarithmic in the window's size. The deployed support stays the full window and the scores are re-evaluated on it. **On since 2026-09-08** (was `0`): validated on a continental problem, where it took the worst rank's fit from 2,015 s to 588 s with the operator and the solve unchanged. Fits are not bitwise identical to 0.2.x on rows above the trigger; set `0` to restore. |
