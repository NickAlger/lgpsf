...
| `operator_fit.hpp` | `fit_operator`, row-parallel, with the always-on baseline guard and CSR fit windows. Window shape is one continuous `window_aspect_cap`. All windows from ONE dual-tree descent. The row body is three phases (package / search / re-score + guard), and an optional `RowDelegate` lets a distributed caller run the middle one on another rank: `assign` after the window pre-pass, `solve` on the packages of the rows it handed away. Absent (the default) the three phases stay fused and nothing is materialized. |
| `row_balance.hpp` | `balance_rows`: which rows a distributed fit should fit somewhere other than their owner, and where. A pure function of (weights, owners, num_ranks, tolerance) -- so every rank computes the same plan and none of it is communicated -- which is why it sits here rather than under `mpi/`, and why the serial suite can test it. Reports the makespan it ACHIEVES, which can exceed the target it aimed at: whole-row granularity means the target is not always reachable. Plan of record: [`row-balance-plan.md`](row-balance-plan.md). |
...
| `corrections/rebuild.hpp` | `rebuild_at(A, a1)`: re-anchor the contracts at a new build shift with NO new $H_d$ access — incremental re-certification (the block deflates what is already found, so only the newly opened flip window is discovered), re-deflation from the archived residuals in the new metric, and `fold_value_pairs` (archived pairs re-orthonormalize in the new metric by linear combination; $H_d$ of the combinations is exact from the stored images — zero new applies). Resumable like `make_pd`. |
| `corrections/cholesky_backend.hpp` | The optional, capability-gated backend — the ONE file allowed to see entries. Activates only when the caller additionally supplies sparse $B$ and $H_r$ (verified stochastically against the struct's handles; a mismatch is refused). `cholesky_solve`: LDLT of $B + aH_r$ per shift (keyed cache that follows the block's rank) with the block correction wrapped by Woodbury in the any-symmetric-$C$ form; `sparse_part_pd`: the exact inertia certificate for the raw sparse pencil. The three-way agreement test (dense / two-level / backend) is the built-in correctness check the plan promised. |
