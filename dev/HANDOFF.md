...
| Heat mini-paper | **slice 1 of 6 landed** (driver skeleton `docs/heat-pipeline/make_figures.py` under the `tttt` env; problem/psfs figures + snippet extraction working; grid 32, noise 1.5%, seed 20260803; NOTE for slices 2+: rebuild `build-py39` bindings at tip for the matplotlib env). Plan — [`heat-mini-paper-plan.md`](heat-mini-paper-plan.md) has every decision (grid 32; noisy data shown; title "LGPSF Heat Inverse Problem Tutorial"; item 8 = three-figure spectrum progression, Fig 8b one-sided singular values for all three curves; cost table = probes vs preconditioned condition number). Next: slice 1 of its slice list (driver skeleton + items 2–4 figures) in `docs/heat-pipeline/` |
