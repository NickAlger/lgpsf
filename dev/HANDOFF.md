...
| Heat mini-paper | **slice 2 of 6 landed** (`docs/heat-pipeline/make_figures.py` under the `tttt` env with `build-py39/bindings` on PYTHONPATH; grid 32, noise 1.5%, seed 20260803). Slice 1: problem/psfs figures + snippet extraction. Slice 2: fit ladder k = 10/20/50/150 (weighted-symmetrized, **spike OFF** — Nick's call, see the plan's decision list: smooth expansion is the main fit, spike is the undermeshed-regime trick and was fitting negative on half the rows at small k; cached in `cache/`, probe pairs saved for the corrections slices), item-5 figure `psf_fit.png` (slab-side interface row chosen median-representative; ~5% of interface rows regress at k = 20 — QC catches them), held-out QC table by region (`snippets/qc_table.tex`; overall median 0.26→0.06, pocket floors at 0.012 from k = 50 — the spike would buy that back). Also fixed slice-1 interface spot labels (at grid 32 the band cells are x = 0.391/0.422). Slice 3: canonical build `corrected_operator` (k = 50 fit + make_pd at A0 = 1e-5 → 3 flips, floor −3.66e-6 + value_pass V2 m = 30 → rank 33) and the a-sweep `regularization_sweep` (logspace 1e-2…1e-7 top-down, TwoLevel-preconditioned outer PCG on the true operator, 5→150 PDE applies; results cached in `cache/sweep.npz`): `lcurve.png` (discrepancy corner a* = 1.3e-5; the vertical arm lives below the certified floor, so `rebuild_at(1e-7)` fires naturally mid-sweep — 144 flips, zero PDE solves, vp pairs refold free) and `recon.png` (too-small/corner/too-large; the corner recovers blob + background + pocket spots, slab spots stay lost = consistent with the data panel). Pencil facts for the paper: 345 negative modes, only 3 below the A0 threshold. Slice 4: items 8–10 at the corner a*. `corner_preconditioners` (cached `precond.npz`) builds every variant dense; `spectra.png` 3 panels (a: reg 3.75e3 vs k-ladder 15.1/6.43/3.51/3.07; b: σ-ladder as-fitted 9.59 / Average 7.58 / Weighted ~3.5; c: free deflation WORSENS the tail — cond 15, the plan's lean-k risk — value pass resolves, 2.45); `snippets/cost_table.tex` = closing cost table (probes vs condition). `convergence.png`: prior 166 its vs 32/17/15/14 — matches spectra. `iterates.png` (ITER_SNAPSHOTS 1,2,3,5,16,40, per-panel residuals): prior coarse-to-fine (background spot arrives ~16–40), lgpsf all-scales-at-once (done by 3–5). Fit caches now store all three assembly policies (`fitted_operator(..., policy=)`). Slice 5: `heat-pipeline.tex` → committed `heat-pipeline.pdf` (9 pp, 8 sections, all 8 figures + 9 extracted snippets/tables wired in via \input; tutorial register, every number with its mechanism). Slice 6: discoverability links (repo README, docs/README "Start here", examples/README corrections intro) + full from-scratch regeneration — `cache/` deleted, every stage rerun, **all figures and snippets came back bit-identical** and every printed number matched. **COMPLETE — all 6 slices.** |
| Window coarsening (per-row work bound) | **PLANNED, 2026-09-06** — [`window-coarsening-plan.md`](window-coarsening-plan.md): a radially graded quadrature coarsening of a row's window (cells ≤ eps × distance from the centre, mass-weighted centroids and probe means, spike and farthest point kept as singletons), applied above a point-count trigger, off by default; motivated by a continental fit where a few hundred rows with priors too wide by decades held ~90k-point windows and cost one rank 5,900 s. Design decisions taken 2026-09-06 (default off, full-window re-score, window-frame grading, public API, protected positions generalizing the spike, a resolution rule for released centres); S1 (prototype + measurements, `experiments/window-coarsening.md`: eps 0.1, first-order error for white-noise probes, the re-score is load-bearing), S2 (`coarsen_window.hpp` + tests) and S3 (`coarsen_above` / `coarsen_eps` on `OperatorFitConfig`, `FitDiagnostics::fit_points`, `DistFitResult::fit_points_total`, the full-window re-score of the finalists, `ProbeFitConfig::resolution_eps`; default off, suite 206 cases green) LANDED 2026-09-06; S4 (bindings: coarsen_window, CoarseWindow, the config fields, fit_points; 71 pytest cases) and S5 (the MPI gate's opt-in --coarsen pass: G-L2 bitwise at n = 1/2/4 with 564/600 rows coarsened) LANDED 2026-09-07; S7 (`docs/window-coarsening-notes.pdf`) and S8 (`examples/coarsened_fit.py` + its page) LANDED 2026-09-07; S6: the sub-mesh part done 2026-09-07 (operator unchanged at eps 0.1 and 0.3 through the consuming driver at 4 ranks); the continental run with the trigger at 3,000 is queued and decides the default; closing slices S7 (a user-facing math note in docs/) and S8 (a narrated single-function example with and without coarsening) added at the maintainer's request  Continental outcome (2026-09-07/08): at the trigger 3,000 with eps 0.1 the worst rank's fit fell from 2,015 s to 588 s at the same prior, with the operator, the QC ladder and the CG count unchanged; DEFAULT CHANGED 2026-09-08: `coarsen_above` is now 3000 in lgpsf, in lgpsf-hessian's `lgh_fit_opts_default` and in ymir's `--pencil-coarsen-above`.  Fits on rows above the trigger are NOT bitwise identical to earlier releases; `0` restores the old path byte for byte.  Suite green after the change (208 cases / 392,025 assertions, 72 pytest). |
| Row-fit load balancing | **PLANNED, 2026-09-08** — [`row-balance-plan.md`](row-balance-plan.md): the fit's wall clock is set by one rank, because the expensive rows cluster spatially (measured on a continental build: one rank of 192 spends 482 s on a rung whose mean is 22 s, and the fit is 976 s of a 1,938 s build). Plan: a FITTING-ONLY redistribution — the coarsened quadrature is packaged on the row's owner, the LM search runs wherever there is capacity, and the candidate parameters come back to the owner, which re-scores on the true window, guards, selects and assembles. The halo, the deployed operator and `dist_fit`'s contract are all untouched, so the consumer needs a knob and nothing else. Rows and destinations come from one water-filling rule (`T = max((1+eps) mean, max weight)`, move each overloaded rank's largest rows until it is under T, place longest-first into the deepest slack) whose migration set is empty when the problem is already balanced. Measured on the same build: moving 2% of rows takes max/mean from 22.2 to 2.4-3.0, moving 5% to 1.3-2.0; a purely local predictor (window area x local node density) tracks the true coarse point count at log-correlation 0.96, so the first rung needs no history. Slices: (1) the phase split with everything still on D1, bitwise against the current gate; (2) the assignment rule as a pure function; (3) migration, default off, with a perverse-assignment gate pass; (4) consumer pass-through and one continental run; (5) weight feedback and a locality tie-break if the field says they are needed. |
| Parallel architecture of the fit | **PARKED, 2026-09-08** — [`parallel-architecture-revisit.md`](parallel-architecture-revisit.md): the maintainer flagged that the row-balance retrofit is more intricate than the win justifies, and asked to revisit the architecture later. The note records the root (a rank owns rows and columns together, so balancing, splitting a row's search, and distributing the re-score all fight it), what a row-task design would change, and what a rewrite would have to preserve — above all bit-identity and the consumer's owner-indexed assembly. Not a decision; revisit after the balance work has field numbers, or when the whole-row granularity floor starts binding at larger allocations. |
