...
int main()
{
    ...
    const double limit = problem.H.row(kImpulseRow).cwiseAbs().maxCoeff();
    {
        ...
        fig.save_png("examples/operator_fit_frog_cxx_truth.png", 480);
    }
    for ( std::size_t i = 0; i < fits.size(); ++i )
    {
        et::Plot2D fig;
        draw_field(fig, problem, impulse_rows[i], limit);
        fig.save_png("examples/operator_fit_frog_cxx_k"
    ...
    // what makes the fit a geometric object and not just a pile of numbers.
    {
        ...
        fig.save_png("examples/operator_fit_frog_cxx_ellipsoids.png", 640);
    }
